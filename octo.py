...
def run_profile(profile_name, data, counter):
    ...
    csv_path = os.path.join(RESULTS_PATH, profile_name + '_' + str(data.Channel[counter]) + '_' + str(
        data.STREAMS[counter]) + '_' + str(data.BANDWIDTH[counter]) + '_' + start_time + ".csv")
    json_path = os.path.join(RESULTS_PATH, profile_name + '_' + str(data.Channel[counter]) + '_' + str(
        data.STREAMS[counter]) + '_' + str(data.BANDWIDTH[counter]) + '_' + start_time + ".json")
    ...
    for repeat_count in range(1, repeat + 1):
        ...
        for profile_count, profile in enumerate(profiles):
            ...
            if profile_count < len(profiles):
                logging.info("Sleeping 10 seconds".format(SLEEP_COUNT))
...
def Pal6_Config(index, profile, data):
    try:
        sys.path.insert(0,
                        "/home/octoscope/Desktop/Upgrade Files/v3.4.40-rc1/octobox-v3.4.40-rc1/octobox/stardust/pyAPI/")
    ...
    pal6 = {
        ...
        "ipAddress": ipaddress,
        "primaryChannel": int(data.Channel[index]),
        "bandwidth": str(data.BANDWIDTH.values.tolist()[index]),
        # BANDWIDTH_20_FIXED,BANDWIDTH_40_ADAPT,BANDWIDTH_80_ADAPT,BANDWIDTH_160_ADAPT
        "apStaMode": 'MODE_STA',
        "ssid": str(data.SSID.values.tolist()[index]),
        ...
        "password": str(int(data.Password.values.tolist()[index])),
        "interface": str(data.WIFi_INTERFACE.values.tolist()[index])
        # INTERFACE_B,INTERFACE_G,INTERFACE_A,INTERFACE_N,INTERFACE_AC,INTERFACE_AX
    }
